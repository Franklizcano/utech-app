...
export function CompanyManagement() {
  ...
  const load = (nextPage = 0) => fetchCompaniesAction(nextPage).then((result) => { setCompanies(result.companies); setHasMore(result.hasMore); setPage(nextPage) }).catch(() => setError("No se pudieron cargar las empresas.")).finally(() => setLoading(false))
  useEffect(() => { load(0) }, [])
